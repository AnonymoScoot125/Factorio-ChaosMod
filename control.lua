...
function addChaosEffect(effect)
	...
	effectEntry.variants = effectEntry.variants or {}
...
function formatPrintedEffectDescription(chaosEntry, effectFunctionTable, event)
	
	local description = {""}
	local gain = chaosEntry.gain or effectFunctionTable.gain or 0
	
	if effectFunctionTable.modifyingValue and not effectFunctionTable.gain then
	...
	table.insert(description, gainTable[gain])
	--for k, v in pairs(effectFunctionTable.description) do
	--	game.print(v)
	--end
	
	table.insert(description, effectFunctionTable.description or chaosEntry.description)
	table.insert(description, "[/color]")
...
function pickRandomChaosEffect(event)
	...
	for try = 1, 10 do
		...
		if not isChaosEffectAlreadyActive(chaosEntry.id) then
			...
			if #chaosEntry.variants > 0 then
				variant = math.random(#chaosEntry.variants)
			end
			
			local effectFunctionTable = chaosEntry.effectFunction(event) or {description = chaosEntry.description}
			
			for k, v in pairs(game.players) do
				
				if settings.get_player_settings(k)["chaos-messages-in-chat"].value then
					
					v.print(formatPrintedEffectDescription(chaosEntry, effectFunctionTable, event))
...
function addEffectToPlayerGUI(player, chaosEntry, effectFunctionTable)
	...
	if player.gui.top.chaos_frame then
			...
			local label = frame.add{type="label", name="effect_"..id.."_label", caption=effectFunctionTable.description or chaosEntry.description}
	...
	for _, player in pairs(game.players) do
		...
		if player.gui.top.chaos_frame then
			...
			if mainFrame.chaos_progressbar then
				...
				mainFrame.chaos_progressbar.value = (event.tick - global.nextChaosTimerRefire + REFIRE_INTERVAL) / REFIRE_INTERVAL
			...
			for k, v in pairs(global.activeChaosEffects) do
				...
				if mainFrame["effect_"..v.id] then
					if mainFrame["effect_"..v.id]["effect_"..v.id.."_progressbar"] then
						mainFrame["effect_"..v.id]["effect_"..v.id.."_progressbar"].value = (v.endOnTick - event.tick) / v.duration
					end
				end
			end
...
function updateActiveChaosEffects(event)
	...
	for k, v in pairs(global.activeChaosEffects) do
		...
		if v.endOnTick <= game.tick then
			...
			if effect.resetFunction then effect.resetFunction(event, v.effectFunctionTable) end
